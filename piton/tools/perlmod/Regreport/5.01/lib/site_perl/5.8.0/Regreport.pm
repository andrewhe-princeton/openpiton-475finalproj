...
my($found_l2_miss) 	= 0;
my($total_l2_outcomes_observed) = 0;
my($l2_misses_observed) = 0;
...
sub vcsgrep {
  ...
  # L2 Cache Stats
  my $l2_outcome_pattern = qr/TILE0 L1\.5.* l2miss\s+(\d)/;
  my $total_l2_outcomes_observed = 0;
  my $l2_misses_observed = 0;

  while(<FUT>) {
    # Original error filters
    if (/$non_vcs_error/) {
      print "EXCLUDED $_" if($debug);
      next;
    }
    elsif(/$error_pattern/i || /mismatch|getting shaky|pass|max cycles/i || /L1_.*cache way/) {
    ...
    # L2 Cache Miss/Hit Analysis
    if (/$l2_outcome_pattern/) {
      $found_l2_miss = 1;
      $total_l2_outcomes_observed++;
      my $miss_status = $1;
      $l2_misses_observed++ if $miss_status == 1;
    }
  }
...
printf "Rishabh: L2_MISS_RATE=\n";
if ($total_l2_outcomes_observed > 0) {
  my $l2_hits_observed = $total_l2_outcomes_observed - $l2_misses_observed;
  my $miss_rate = ($l2_misses_observed / $total_l2_outcomes_observed) * 100;

  print "\n--- L2 Cache Statistics (Based on Return Messages) ---\n";
  print "Total L2 Outcomes Observed:  $total_l2_outcomes_observed\n";
  print "L2 Misses Observed:          $l2_misses_observed\n";
  print "L2 Hits Calculated:          $l2_hits_observed\n";
  printf("L2 Miss Rate:                %.2f%%\n", $miss_rate);
} else {
  print "\nWarning: No L2 outcomes found in log file.\n";
} 
print "Rishabh: sim.log check = ", (-r 'sim.log' || -r 'sim.log.gz') ? "true\n" : "false\n";
print "[Rishabh] vcslog_exists set to ", (defined $vcslog_exists ? $vcslog_exists : 'undef'), "\n" if $debug;
